...
async function createDocType(doctypeName: any, moduleName: any, fields:any) {
  const docFields: DocField[] = fields.map((field: any) => ({
...
async function creatDocTypeFromInterface(iface: InterfaceDeclaration, is_childtable: boolean): Promise<JSON>{
  let properties = iface.getProperties()
  let docfields = properties.map(prop => createDocFieldFromProperty(prop));
  let link_and_table_dt = docfields.filter(field => field.fieldtype === 'Link' || field.fieldtype === 'Table');
  return frappe.call().post('frappe_prompt.api.utils.create_doctype', 
    {"data": {
      "doctype_name": iface.getName(), "module": "Frappe Prompt", "fields": docfields,
      "is_childtable": is_childtable
    }}
  )
...
interfaces.forEach((iface) => {
  creatDocTypeFromInterface(iface);
